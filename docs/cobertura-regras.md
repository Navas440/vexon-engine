...
Auditoria de quanto das regras de `Readme.txt` (livro do jogador) estão de fato implementadas no motor, feita comparando o texto do livro com `backend/classData.js`, `backend/engine/combatEngine.js`, `backend/engine/entityTurn.js`, `backend/engine/restEngine.js`, `backend/engine/deathEngine.js` e `backend/engine/skillEngine.js`.
...
O que passou de ❌ para ✅ nesta tarefa: **Descanso** (curto/longo, com Dados de Vida reais em vez do heal fixo de 50%) e **Teste contra a Morte** (loop completo de sucessos/falhas, incluindo o caso especial de dano a 0 HP e a ação de estabilizar um aliado). Tudo o mais listado acima — perícias oficiais completas, condições, os 8 pontos de recurso de classe distintos, e as 12 árvores de subclasse — continua fora do escopo desta passada e seria, cada um, um projeto à parte (principalmente os pontos de recurso: são 8 sistemas de economia de ação diferentes, um por classe, mais o Medidor de Calor e o Ciclo das Marés que nem são pools simples).
